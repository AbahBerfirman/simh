...
@section('heading', 'View Harga')

@section('right_top_button')
<a href="{{ route('admin_harga_add') }}" class="btn btn-primary"><i class="fa fa-plus"></i> Add New</a>
@endsection
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table table-bordered" id="example1">
                            ...
                            <tbody>
                                @php $i=0; @endphp
                                @foreach($hargas as $row)
                                @php $i++; @endphp
                                <tr>
                                    <td>{{ $loop->iteration }}</td>
                                    <td>{{ $row->hargaKamar->kamarAlamat->alamatKota->name }}</td>
                                    <td>{{ $row->hargaKamar->kamarAlamat->alamatApartement->name }}</td>
                                    <td>{{ $row->hargaKamar->no_kamar }}</td>
                                    <td>{{ $row->hargaDuration->duration }} Jam</td>
                                    <td>{{ $row->harga }}</td>
                                    <td class="pt_10 pb_10">

                                        {{-- <button class="btn btn-warning" data-toggle="modal" data-target="#exampleModal{{ $i }}">Detail</button> --}}

                                        {{-- <a href="{{ route('admin_harga_gallery',$row->id) }}" class="btn btn-success">Gallery</a> --}}

                                        <a href="{{ route('admin_harga_edit',$row->id) }}" class="btn btn-primary">Edit</a>
                                        <a href="{{ route('admin_harga_delete',$row->id) }}" class="btn btn-danger" onClick="return confirm('Are you sure?');">Delete</a>
                                    </td>
                                </tr>
